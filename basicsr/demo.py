...
from basicsr.utils import (get_time_str, ProgressBar, save_config,
                           tensor2img, imwrite, init_loggers, parse_args_demo)
...
def main():
    ...
    clip_name = opt['datasets']['test'].get('clip_name', None)
    ...
    if with_metrics:
        ...
        if opt['rank'] == 0:
            opt_metric = deepcopy(opt['test']['metrics'])
            for name, opt_ in opt_metric.items():
                mean_metric = metrics[name].mean()
                logger.info(f'Mean {name.upper()} on demo {clip_name}: {mean_metric:.4f}')
